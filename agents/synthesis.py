...
_COACH_RULE_GUIDANCE = {
    "rule_3_new_user": (
        "【教练规则指导】\n"
        "⚠️ 新用户首次交互，零风险入口——只肯定，不评判，不给改进建议。\n"
        "suggestion 层改为鼓励尝试新的简单主题（如'下次可以试试画一个苹果'），"
        "不给技巧建议。\n"
        "语气要特别温柔，让用户感到安全和被接纳。"
    ),
...
def _build_synthesis_prompt(
    ...
    layers_spec = (
        '    {\n'
        '      "type": "identify",\n'
        f'      "content": "先认出画的是什么（物体/人物/场景），表现出你看懂了。然后真诚地夸一个具体的亮点。称呼用户为{user_name}。2句话内，精炼有温度。必须用 **加粗** 强调关键技巧或优点，如 **排线**、**透视**。每层最多1处加粗。",\n'
    ...
    layers_spec += (
        ...
        '    {\n'
        '      "type": "encourage",\n'
        '      "content": "以真诚的鼓励收尾，并自然地引出下次可以尝试的方向。2句话内。"\n'
        '    }\n'
    )
